...
import { 
  ...
  Building2, Landmark, Wallet, Scale, FileCheck, X
} from 'lucide-react';
...
export const Sidebar: React.FC<SidebarProps> = ({ userRole, activeTab, onSelectMenu, isOpenMobile = false, onCloseMobile }) => {
  ...
  return (
    <>
      ...
      {/* Sidebar Container (Slide-over on mobile, static on desktop) */}
      <aside className={`
        fixed inset-y-0 left-0 z-50 w-72 bg-slate-900 border-r border-slate-800 text-slate-300 flex flex-col h-full select-none shadow-2xl transition-transform duration-300 ease-in-out
        lg:static lg:w-64 lg:z-30 lg:translate-x-0 lg:shadow-none
      ...
      `}>
        ...
        <div className="flex-1 overflow-y-auto py-3 px-3 space-y-4 custom-scrollbar">
          ...
          <button
            onClick={() => handleItemClick('dashboard', 'Beranda')}
